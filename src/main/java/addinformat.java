...
@WebServlet("/addinformat")
public class addinformat extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Dao dao=new Dao();
        try {
            if(dao.addinformat(data)) {
                response.sendRedirect("operator.jsp");
            }
            else {
                request.getRequestDispatcher("addinformat.jsp").forward(request, response);
            }
